...
export async function generateRestaurantInsights(restaurantId: number): Promise<AIInsight[]> {
  try {
    ...
    const prompt = `
      ...
      - Most Popular Items: ${restaurantData.popularItems?.map(item => `${item.name} (${item.count} orders)`).join(', ') || 'No data available'}
...
async function generateDataDrivenInsights(restaurantId: number): Promise<AIInsight[]> {
  ...
  if (restaurantData.revenue && restaurantData.revenue < 1000) {
    insights.push({
      type: 'revenue',
      title: 'Revenue Growth Opportunity',
      description: `Current monthly revenue of $${restaurantData.revenue?.toFixed(2) || '0.00'} shows significant growth potential. Focus on increasing order frequency and average order value.`,
      recommendations: [
      ...
      dataSource: {
        ...
        value: `$${restaurantData.revenue?.toFixed(2) || '0.00'}`,
        period: 'last 30 days'
      },
      confidence: 85,
      priority: 'high',
  ...
  if (restaurantData.totalOrders && restaurantData.totalOrders < 50) {
    insights.push({
      type: 'marketing',
      ...
      dataSource: {
        metric: 'Total Orders',
        value: restaurantData.totalOrders || 0,
        period: 'last 30 days'
      },
      confidence: 90,
      priority: 'high',
  ...
  if (restaurantData.popularItems && restaurantData.popularItems.length > 0) {
  ...
  const utilizationRate = ((restaurantData.activeTables || 0) / (restaurantData.totalTables || 1)) * 100;
