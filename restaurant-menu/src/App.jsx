...
import { Routes, Route, useParams, useNavigate, useLocation, BrowserRouter } from "react-router-dom";
  ...
  useEffect(() => {
    ...
    async function createSessionAndFetchMenu() {
      ...
      try {
        ...
        if (!sessionRes.ok) {
          const errorData = await sessionRes.json().catch(() => ({ message: "Failed to create table session" }));
          throw new Error(errorData.message);
        }
        const sessionData = await sessionRes.json();
        const newTableSessionId = sessionData.id;
        setTableSessionId(newTableSessionId);

        // 2. Create customer for the session
        const customerRes = await fetch(`/api/public/restaurants/${restaurantId}/customers`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (!customerRes.ok) {
          const errorData = await customerRes.json().catch(() => ({ message: "Failed to create customer" }));
          throw new Error(errorData.message);
        }
